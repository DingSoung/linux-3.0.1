...
int s3c_tvenc_open(struct file *filp) 
{ 
	int err;
	err = video_exclusive_open(filp);	// One function of V4l2 driver
...
int s3c_tvenc_release(struct file *filp) 
{
	video_exclusive_release(filp);
...
//static int s3c_tvenc_do_ioctl(struct file *filp,unsigned int cmd,void *arg)
{
	int ret;
	switch(cmd){
...
static int s3c_tvenc_ioctl_v4l2(struct file *filp, unsigned int cmd,
		    unsigned long arg)
{
	return video_usercopy(filp, cmd, arg, s3c_tvenc_do_ioctl);
}
...
struct v4l2_file_operations s3c_tvenc_fops = {
//struct file_operations s3c_tvenc_fops = {
	.owner = THIS_MODULE,
	.open = s3c_tvenc_open,
	.unlocked_ioctl = s3c_tvenc_ioctl_v4l2,
 ...
 */
int video_exclusive_open(struct file *file)
{
...
int video_exclusive_release(struct file *file)
{
	struct  video_device *vfl = video_devdata(file);
