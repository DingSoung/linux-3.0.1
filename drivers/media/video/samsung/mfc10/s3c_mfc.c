...
static int s3c_mfc_ioctl(/*struct inode *inode,*/ struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case S3C_MFC_IOCTL_MFC_H263_ENC_EXE:
		...
		size = tmp * pMfcInst->yuv_buffer_count; 
                //dma_cache_maint(start, size, DMA_TO_DEVICE);static struct miscdevice s3c_mfc_miscdev
                dma_map_single(NULL,start, size, DMA_TO_DEVICE);
		...
		//dma_cache_maint(start, size, DMA_FROM_DEVICE);
                dma_map_single(NULL, start, size, DMA_FROM_DEVICE);
...
static void *Phy2Vir_AddMapping(unsigned int phy_addr, int meme_size)
{
	void *reserved_mem;
...
static int s3c_mfc_probe(struct platform_device *pdev)
{
	...
	// cym add
	printk("->################### s3cmfc:s3c_mfc_sfr_base_virt_addr:%x",s3c_mfc_sfr_base_virt_addr);
